'''
stateful能够保证批次之间的状态时有传递的，这对于训练巨长文本非常有用。

给定一个序列，如[1,2,3,4,5,6],假设均值步长为2，则应该输出[1.5,2.5,3.5,4.5,5.5]
训练LSTM时，每个样本片段长度为3，则：
1,2,3->1.5,2.5
2,3,4->2.5,3.5
...
model_stateless.fit(x_train,
                    ...
                    validation_data=(x_test, y_test),
                    shuffle=False,
                    callbacks=[EarlyStopping(monitor='acc', patience=3, baseline=0, mode='max', min_delta=0.001)])
...
plt.plot((y_test - predicted_stateful).flatten()[tsteps - 1:], label='Stateful: Expected - Predicted')
plt.plot((y_test - predicted_stateless).flatten(), label='Stateless: Expected - Predicted')
plt.legend()
plt.show()
